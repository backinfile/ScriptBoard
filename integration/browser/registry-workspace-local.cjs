...
(async()=>{
 ...
 try{
  ...
  await root.locator(':scope > summary > a').click();await page.waitForURL('**/*namespace=jiangnan%2F');await page.locator('.registry-namespaces a[aria-current]').waitFor();await root.locator(':scope > summary > a').click();
  const main=root.locator('details').filter({has:page.locator(':scope > summary').filter({hasText:'main'})}).first();
  await main.locator(':scope > summary > a').click();
  ...
  assert.equal(await page.locator('.registry-namespaces details[open]').count(),1);checks.push('nested namespace collapse, selection, ancestor expansion and sibling boundary');
