...
(async () => {
  ...
  await page.addScriptTag({ path: path.join(repository, "internal/web/ui/assets/app.js") });
	await page.locator(".app-sidebar").evaluate((element) => { element.style.pointerEvents = "none"; });
  ...
  assert.equal(await drawer.evaluate((element) => element.classList.contains("is-closing")), false);
	...
	assert.equal(publicPalette.sidebar, "rgb(20, 22, 28)", "monitor dashboards must retain the shared application-shell sidebar palette");
