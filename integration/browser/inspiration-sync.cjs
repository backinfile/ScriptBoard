...
module.exports=async function verifySync(admin,viewer,baseURL){
 ...
 for(const p of [admin,viewer])await p.locator('.wb-status').filter({hasText:'已保存'}).waitFor();
 ...
 await field(viewer,'b').fill('Remote B while Alice types');await viewer.locator('.wb-status').filter({hasText:'已保存'}).waitFor();
 ...
 await field(viewer,'a').focus();await field(viewer,'a').fill('Remote A conflict');await viewer.locator('.wb-status').filter({hasText:'已保存'}).waitFor();
 ...
 await admin.locator('[data-id="'+id+'-a"]').getByRole('button',{name:'保存本地版本',exact:true}).click();await admin.locator('.wb-status').filter({hasText:'已保存'}).waitFor();assert.equal((await read(admin)).boards.find(b=>b.id===id).items.find(w=>w.id===id+'-a').text,'Local A preserved');
 ...
 await field(viewer,'b').fill('Saved during disconnect');await viewer.locator('.wb-status').filter({hasText:'已保存'}).waitFor();await admin.context().setOffline(false);
