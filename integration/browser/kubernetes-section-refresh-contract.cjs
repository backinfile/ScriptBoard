...
const snapshot = version => '<!doctype html><html><body><main data-kubernetes-page data-kubernetes-refresh-failed="Refresh failed; previous data preserved" data-kubernetes-refreshed="Updated"><div class="kubernetes-monitor-summary">cluster '+version+'</div><div data-kubernetes-alert-slot>cluster alert '+version+'</div><button data-kubernetes-expand-all>Expand all</button><button data-kubernetes-collapse-all>Collapse all</button>'+["external","workloads","nodes"].map(id=>'<details data-kubernetes-section="'+id+'"><summary>'+id+'<span data-kubernetes-section-facts="'+id+'">'+version+'</span></summary><div><button type="button" data-kubernetes-refresh aria-label="Refresh resources">Refresh</button><span data-kubernetes-refresh-status role="status"></span>'+(id==="external"?'<div data-kubernetes-external-body>service '+version+'</div>':id==="workloads"?'<div class="kubernetes-workload-controls">filters '+version+'</div><div class="kubernetes-table-shell">workload '+version+'</div>':'<div data-kubernetes-node-list>node '+version+'</div>')+'</div></details>').join("")+'</main></body></html>';
(async()=>{
 ...
 try {
  ...
  await button.click();
  await page.waitForFunction(()=>document.querySelectorAll("[data-kubernetes-refresh]:disabled").length===3);
  ...
  // Every section's control refreshes the same three resources, including closed sections.
  for(const [id,version] of [["workloads",3],["nodes",4]]){
   await page.locator('[data-kubernetes-section="'+id+'"] summary').click();
   await page.locator('[data-kubernetes-section="'+id+'"] [data-kubernetes-refresh]').click();
  ...
  console.log("PASS Kubernetes disclosures: three scoped refresh triggers, busy state, preserved disclosures/URL/summary, atomic failure and retry, bulk toggles, keyboard.");
