...
module.exports=async function verifySharedSpace(page,baseURL,screenshot){
 ...
 await page.goto(baseURL+'/resources/databases');const {fontSize:databaseFontSize,...databaseStyle}=await page.evaluate(style);const rem=await page.evaluate(()=>parseFloat(getComputedStyle(document.documentElement).fontSize));assert.ok(Math.abs(parseFloat(databaseFontSize)-2.2*rem)<0.01);
 ...
 await page.goto(baseURL+'/resources/workbench');await page.locator('.wb-enhanced').waitFor();assert.equal(await page.locator('main h1').first().textContent(),'灵感空间');
 // Verify the shared header layout while retaining Inspiration space's own heading scale.
 const {fontSize:workbenchFontSize,...workbenchStyle}=await page.evaluate(style);assert.ok(Math.abs(parseFloat(workbenchFontSize)-2.4*rem)<0.01);assert.deepEqual(workbenchStyle,databaseStyle);
