...
module.exports=async function verifySharedSpace(page,baseURL,screenshot){
 ...
 const style=()=>{const n=document.querySelector('main h1'),s=getComputedStyle(n),h=n.closest('header');return {fontSize:s.fontSize,fontWeight:s.fontWeight,lineHeight:s.lineHeight,padding:getComputedStyle(h).padding};};
 ...
 await viewer.getByRole('button',{name:'编辑',exact:true}).click();await viewer.getByRole('button',{name:'新建面板',exact:true}).click();await viewer.getByRole('textbox',{name:'面板名称',exact:true}).fill('全员共享验收 '+username);await viewer.locator('[data-add=note]').click();await viewer.locator('.wb-note textarea').fill('来自另一位登录成员的记录');await viewer.locator('.wb-status').filter({hasText:'已保存'}).waitFor();
