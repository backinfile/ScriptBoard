...
(async()=>{
 ...
 try {
  ...
  await panel.locator('[name=name]').fill('本地生产仓库');await panel.locator('[name=endpoint]').fill(registry);await panel.locator('[name=access_mode]').selectOption('write');await panel.getByRole('button',{name:'保存连接'}).click();await page.locator('.registry-image-link').first().waitFor();await page.locator('.task-panel-host.is-open .task-panel').waitFor({state:'detached'});
  const selectedURL=page.url();const id=new URL(selectedURL).searchParams.get('connection');
  await page.getByRole('link',{name:'添加仓库',exact:true}).first().click();await panel.locator('[name=name]').fill('开发仓库');await panel.locator('[name=endpoint]').fill(registry);await panel.locator('[name=access_mode]').selectOption('write');await panel.getByRole('button',{name:'保存连接'}).click();await panel.waitFor({state:'detached'});await page.locator('.registry-connection[href='+JSON.stringify(new URL(selectedURL).pathname+new URL(selectedURL).search)+']').click();await page.waitForURL(selectedURL);checks.push('connection switching');
  await page.screenshot({path:path.join(output,'desktop.png'),fullPage:true});
  await page.locator('.registry-namespaces summary').filter({hasText:/^team-a$/}).click();await page.locator('.registry-namespaces a').filter({hasText:/^team-a\/$/}).click();await page.waitForURL('**/*namespace=team-a%2F');assert.equal(await page.locator('.registry-image-link').count(),2);assert.equal(await page.locator('.registry-image-link').filter({hasText:'team-ab/api'}).count(),0);checks.push('namespace excludes sibling prefix');
  ...
  for(const repo of ['team-a/api','team-a/jobs/worker']){const result=await context.request.get(registry+'/v2/'+repo+'/tags/list');assert.deepEqual((await result.json()).tags || [],[])}
