...
func TestTypedBuiltinCrossesRunnerTransport(t *testing.T) {
	if testing.Short() {
		t.Skip("builds a deterministic Runner fixture")
	}
	root := t.TempDir()
	scriptPath := filepath.Join(root, "job.sbflow")
	script := []byte("{\"Uses\":\"write-file\",\"With\":{\"path\":\"result.txt\",\"content\":\"runner-builtin\"}}")
	if err := os.WriteFile(scriptPath, script, 0o600); err != nil {
		t.Fatal(err)
	}
	digest := sha256.Sum256(script)
	...
	stdout, stderr := <-stdoutDone, <-stderrDone
	if !strings.Contains(stdout, "wrote") || stderr != "" || gotExecutor != "scriptboard-builtin" {
		t.Fatalf("stdout=%q stderr=%q executor=%q", stdout, stderr, gotExecutor)
	}
	data, err := os.ReadFile(filepath.Join(root, "result.txt"))
	if err != nil || string(data) != "runner-builtin" {
