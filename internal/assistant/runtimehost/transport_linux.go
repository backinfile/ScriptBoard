...
func DefaultEndpoint(stateRoot string) (string, error) {
	...
	return filepath.Join("/run/scriptboard-ai", "runtime-"+id+".sock"), nil
}
...
func Listen(options TransportOptions) (*Transport, error) {
	...
	if !filepath.IsAbs(endpoint) || len(endpoint) >= 100 {
		return nil, errors.New("Runtime Host Unix socket path must be a short absolute path")
	}
	allowedUID, err := allowedLinuxUID(options)
	if err != nil {
		return nil, err
	}
	listener, activated, err := systemdactivation.Listener("scriptboard-ai", endpoint)
	if err != nil {
		return nil, err
	}
	if !activated {
		listener, err = listenRuntimeUnix(endpoint)
		if err != nil {
			return nil, err
		}
	}
	...
	verify := func(connection net.Conn) error {
		...
		if !ok {
			return errors.New("Runtime Host peer is not a Unix socket")
		}
		...
		if credential == nil || int(credential.Uid) != allowedUID {
			return errors.New("Runtime Host peer UID is not authorized")
		}
		return nil
	}
...
func listenRuntimeUnix(endpoint string) (net.Listener, error) {
	...
	if info, statErr := os.Lstat(endpoint); statErr == nil {
		if info.Mode()&os.ModeSocket == 0 {
			return nil, errors.New("Runtime Host endpoint exists and is not a socket")
		}
...
func allowedLinuxUID(options TransportOptions) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("look up Runtime Host Web identity %q: %w", identity, err)
	}
	uid, err := strconv.Atoi(account.Uid)
	if err != nil || uid < 0 {
		return 0, fmt.Errorf("Runtime Host Web identity %q has an invalid UID", identity)
	}
	return uid, nil
}
