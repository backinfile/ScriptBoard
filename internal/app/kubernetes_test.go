...
func TestKubernetesPageConfiguresTheOnlyClusterAndListsWorkloads(t *testing.T) {
	...
	page, _ = io.ReadAll(response.Body)
	_ = response.Body.Close()
	for _, expected := range [][]byte{[]byte("edge-home"), []byte("ghcr.io/acme/api:v2"), []byte("production"), []byte("Connection settings"), []byte(`href="/monitor/kubernetes?direction=asc&amp;sort=name"`), []byte(`class="kubernetes-workload-controls"`), []byte(`class="monitor-status-switch kubernetes-status-tabs"`), []byte(">Ready<")} {
