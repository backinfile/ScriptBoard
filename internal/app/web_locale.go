...
var webMessages = map[string]localizedMessage{
	...
	"shell.stopped_pinned":                          {"已停止的 Pin 应用", "Stopped pinned applications"},
	"shell.application_attention":                   {"应用观测需要关注", "Application observation needs attention"},
	"shell.application_issues":                      {"应用采集异常", "Application collection issues"},
	"shell.review_applications":                     {"查看已 Pin 应用", "Review pinned applications"},
	...
	"applications.pin_live_update":                  {"Pin 实时更新", "Pinned live updates"},
	...
	"applications.running_description":              {"选择应用可在右侧查看运行事实；默认按 CPU 降序显示前 100 个应用。", "Select an application to inspect runtime facts in the side panel. The first 100 are shown by CPU descending."},
	...
	"applications.runtime_tab_description":          {"进程、容器与启动命令的客观事实", "Process, container, and startup facts"},
	"applications.history_description":              {"查看所选时间范围内的 CPU、内存和磁盘变化。", "Review CPU, memory, and disk changes in the selected time range."},
	...
	"applications.refresh_runtime":                  {"刷新运行信息", "Refresh runtime facts"},
	"website.title":                                 {"网站监控", "Website Monitoring"},
	"website.eyebrow":                               {"监控 / 网站", "Monitor / Websites"},
	"website.description":                           {"从当前主机检查本机服务和外部网站，集中查看可用性、响应证据与连接安全。", "Check local services and external websites from this host, with availability, response evidence, and connection security in one place."},
	...
	"website.recent_checks_healthy":                 {"最近一次检查均未发现可用性故障。", "The latest checks found no availability failures."},
	...
	"website.result_needs_review":                   {"检查结果需要管理员确认。", "The check result needs administrator review."},
	...
	"website.ledger":                                {"完整网站状态台账", "Complete website status ledger"},
	...
	"website.empty_filtered_description":            {"清除筛选后可查看完整台账。", "Clear the filters to view the complete ledger."},
	"website.empty_title":                           {"还没有网站监控", "No website monitors yet"},
	"website.empty_description":                     {"手动新增一个网站，或主动扫描本机 Nginx 配置后选择要加入的候选项。", "Add a website manually, or scan the local Nginx configuration and choose candidates to import."},
	...
	"website.form.edit_description":                 {"保存后会使旧检查结果失效，并立即使用新配置检查一次。", "Saving invalidates the previous check result and immediately runs a check with the new configuration."},
	"website.form.add_description":                  {"保存后立即从当前主机检查一次。目标当前不可用也不会阻止保存。", "Saving immediately runs a check from this host. A currently unavailable target will not prevent saving."},
	...
	"website.form.scope_hint":                       {"用于筛选；Nginx 导入会自动配置本机直连地址。", "Used for filtering. Nginx imports automatically configure a local direct-connect address."},
	...
	"website.nginx.description":                     {"ScriptBoard 只读取配置，不会启动 Nginx、扫描端口、修改配置或重新加载服务。", "ScriptBoard only reads configuration. It does not start Nginx, scan ports, modify configuration, or reload the service."},
	...
	"website.nginx.view_ledger":                     {"查看网站台账", "View website ledger"},
	"website.nginx.preview_description":             {"查找结果不会自动加入监控。勾选确认后，ScriptBoard 才会保存所选网站。", "Scan results are not added automatically. ScriptBoard saves only the websites you select and confirm."},
	...
	"website.nginx.none_description":                {"查看扫描说明，或返回修改配置文件路径后重试。", "Review the scan notes, or change the configuration path and try again."},
	...
	"website.nginx.config_hint":                     {"必须填写绝对路径；留空时从运行中 Nginx 的 -p / -c 参数与平台默认位置查找。", "Use an absolute path. Leave blank to search running Nginx -p / -c arguments and platform defaults."},
	...
	"website.detail.refreshed_in_place":             {"内容已在当前抽屉中更新", "Updated in the current drawer"},
	...
	"website.detail.current_verdict":                {"当前判断", "Current verdict"},
	...
	"website.detail.last_24h_description":           {"空白表示 ScriptBoard 没有执行检查，不会被计作成功或失败。", "A gap means ScriptBoard did not run a check; it is not counted as success or failure."},
	...
	"website.detail.incident_confirmed_description": {"连续检查已满足事故确认条件；恢复前将继续复核。", "Consecutive checks confirmed this incident; verification continues until recovery."},
	"website.detail.verification_description":       {"本次失败尚待下一次检查确认，不会提前记为事故。", "The next check must confirm this failure before it is recorded as an incident."},
	"website.detail.recent_checks":                  {"最近 5 次检查", "Five most recent checks"},
	"website.detail.recent_checks_description":      {"逐次检查证据按时间倒序排列。", "Individual check evidence, newest first."},
	"website.detail.no_recent_checks":               {"还没有检查记录。", "No check results yet."},
	"website.detail.latest_description":             {"管理员判断问题影响时优先使用这些事实。", "Use these facts first when assessing the impact of a problem."},
	...
	"website.detail.security_description":           {"来自最近一次成功建立的 TLS 连接。", "From the most recently established TLS connection."},
	...
	"website.detail.incidents_description":          {"确认故障保留一年；暂停或删除会写明关闭原因。", "Confirmed incidents are retained for one year. Pausing or deleting records the closure reason."},
	...
	"website.detail.delete_description":             {"网站会从活动台账中移除；既有检查与故障记录仍按保留期保存。", "The website is removed from the active ledger. Existing checks and incidents remain until their retention periods expire."},
	...
	"website.security.unavailable_description":      {"连接成功后，ScriptBoard 会显示证书颁发机构、有效期和 TLS 版本。", "After a successful connection, ScriptBoard shows the issuer, validity period, and TLS version."},
	...
	"settings.description":                          {"管理账户、状态显示、本地版本保护与应用更新。", "Manage the account, status display, local version protection, and application updates."},
	"settings.sections":                             {"设置分区", "Settings sections"},
	"settings.account":                              {"账户", "Account"},
	"display.title":                                 {"状态显示", "Status display"},
	"display.description":                           {"调整状态的视觉表达；监控结论和故障判定不会受这些设置影响。", "Adjust how status is presented. Monitoring conclusions and failure detection are unchanged."},
	"display.website_status_color":                  {"网站监控故障色", "Website monitoring fault color"},
	"display.website_status_color_description":      {"正常状态固定使用绿色；在这里选择已确认故障的强调色。颜色始终与图标和文字同时使用。", "Healthy status always uses green. Choose the emphasis color for confirmed failures. Color is always paired with an icon and text."},
	"display.red":                                   {"清晰红", "Clear red"},
	"display.red_description":                       {"与正常绿色形成直接、熟悉的语义对比。", "A direct, familiar semantic contrast with healthy green."},
	"display.magenta":                               {"高辨识洋红", "High-contrast magenta"},
	"display.magenta_description":                   {"进一步拉开与绿色的视觉距离，适合红绿色觉辨识较弱的显示环境。", "Creates more visual distance from green for displays where red and green are harder to distinguish."},
	"display.browser_preference":                    {"此偏好会立即保存到当前浏览器，并应用到所有网站监控页面。", "This preference is saved immediately in this browser and applied to every website monitoring page."},
