...
func TestAIWorkspaceAndSettingsUsePersistedLLMAndConversationState(t *testing.T) {
	...
	for _, expected := range []string{`data-connection-ok="false"`, `data-state="not-ok"`, `class="button button--compact assistant-llm-test"`, `data-connection-test`, `data-test-kind="llm"`, `>Test not passed<`, `>Test connection<`} {
		if !strings.Contains(string(settings), expected) {
