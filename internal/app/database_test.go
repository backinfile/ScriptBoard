...
func TestOpenDatabaseMigratesWebsiteMonitoringSchema(t *testing.T) {
	...
	if _, err := legacy.Exec(`CREATE TABLE legacy_marker (value TEXT);
		INSERT INTO legacy_marker VALUES ('schema-8');
	...
	db, err := openDatabase(path)
	if err != nil {
		t.Fatalf("migrate schema 8 database: %v", err)
	}
	defer db.Close()
	...
	if _, err := os.Stat(path + ".pre-migration-v8"); err != nil {
		t.Fatalf("schema 8 snapshot: %v", err)
	}
}
