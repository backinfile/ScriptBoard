...
func (a *App) externalInterfacesPage(response http.ResponseWriter, request *http.Request) {
	...
	requests, err := a.externalTriggers.ListInvocations(request.Context(), 50)
	if err != nil {
	...
	for _, key := range keys {
		...
		for _, entry := range key.Entries {
			...
			entries[key.ID] = append(entries[key.ID], externalEntryView{
	...
	if err := externalInterfacesTemplate.Execute(response, externalInterfacesPageData{
		Keys: views, Entries: entries, Requests: requests, CSRFToken: current.csrfToken, Locale: locale, Now: now,
	}); err != nil {
...
func externalTargetText(entry externaltrigger.Entry) string {
	...
	case externaltrigger.ActionLog:
		...
		if entry.DecodeConfig(&config) == nil {
			if config.Category == "" {
				return fmt.Sprintf("%d B", config.MaxMessageBytes)
			}
			return fmt.Sprintf("%s · %d B", config.Category, config.MaxMessageBytes)
		}
	}
	return entry.Target
}
...
func (a *App) createExternalKey(response http.ResponseWriter, request *http.Request) {
	...
	renderExternalInterfaceForm(response, externalInterfaceFormData{
		Kind: "key-secret", Title: webText(locale, "external.key_created"), Description: webText(locale, "external.key_created_description"),
...
func (a *App) updateExternalKey(response http.ResponseWriter, request *http.Request) {
	...
	if label == "" || len([]byte(label)) > 128 || !utf8.ValidString(label) {
		http.Error(response, "invalid key label", http.StatusBadRequest)
		return
	}
...
func (a *App) rotateExternalKey(response http.ResponseWriter, request *http.Request) {
	...
	locale := resolveWebLocale(request)
	response.WriteHeader(http.StatusCreated)
	renderExternalInterfaceForm(response, externalInterfaceFormData{Kind: "key-secret", Title: webText(locale, "external.key_rotated"), Description: webText(locale, "external.key_created_description"), BackURL: "/config/external-interfaces", Locale: locale, Key: key, Secret: secret})
}
...
func (a *App) createExternalEntry(response http.ResponseWriter, request *http.Request) {
	...
	if _, err := a.externalTriggers.Key(request.Context(), keyID); err != nil {
		http.Error(response, "External Interface key not found", http.StatusNotFound)
		return
	}
...
func (a *App) externalEntryConfig(request *http.Request, actionType externaltrigger.ActionType) (any, string, error) {
	...
	case externaltrigger.ActionLog:
		...
		return externaltrigger.LogConfig{Category: strings.TrimSpace(request.FormValue("log_category")), MaxMessageBytes: limit}, "", nil
	case externaltrigger.ActionUpload:
...
func (a *App) executeExternalAction(response http.ResponseWriter, request *http.Request, entry externaltrigger.Entry) externalActionResult {
	switch entry.Type {
	case externaltrigger.ActionLog:
		return executeExternalLog(response, request, entry)
	case externaltrigger.ActionUpload:
...
func executeExternalLog(response http.ResponseWriter, request *http.Request, entry externaltrigger.Entry) externalActionResult {
