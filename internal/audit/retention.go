// Package audit owns audit lifecycle rules that are independent of transport.
package audit
...
func CleanupExpiredEvents(db *sql.DB, stateRoot string, cutoff time.Time) (int, error) {
	return CleanupExpiredEventsBefore(db, stateRoot, cutoff, 0)
}

func CleanupExpiredEventsBefore(db *sql.DB, stateRoot string, cutoff time.Time, preserveFromAuditID int64) (int, error) {
	chainEnabled := retentionChainEnabled(db)
	hashColumn := "''"
	if chainEnabled {
...
func retentionChainEnabled(db *sql.DB) bool {
