...
func RunRunner(ctx context.Context, arguments []string, getenv func(string) string) error {
	...
	absolute, err := absoluteStateRoot(*stateRoot, "Runner Host")
	if err != nil {
		return err
	}
	...
	loaded, err := config.Load(RunnerConfigArguments(*configPath, absolute), getenv)
	if err != nil {
		return err
	}
