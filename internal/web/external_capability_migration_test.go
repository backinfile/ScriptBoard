...
func TestSchema36ExternalKeysSplitIntoSingleEntryCapabilities(t *testing.T) {
	...
	// Group-owned paths intentionally allow one key to authorize multiple
	// entries; the legacy one-entry-per-key constraint must not survive schema 46.
	if _, err := database.Exec(`INSERT INTO external_trigger_entries
		...
		t.Fatalf("migrated group rejected a second key-authorized Entry: %v", err)
	}
}
