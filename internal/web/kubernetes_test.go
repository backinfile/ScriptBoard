...
func TestKubernetesPageSeparatesConnectionsFromSelectedClusterMonitoring(t *testing.T) {
	fixture := &kubernetesFixtureClient{
		snapshot: clusterstatus.Snapshot{
			...
			Workloads: []clusterstatus.Workload{{Key: "production/Deployment/api", Namespace: "production", Kind: "Deployment", Name: "api", Image: "ghcr.io/acme/api:v2", Status: "ready", StatusLabel: "正常", Ready: 2, Desired: 2, CPUMillicores: 200, MemoryBytes: 224 << 20, Nodes: "edge-worker-01", Revision: "rev 2", LastStartedAt: time.Date(2026, 9, 14, 4, 0, 0, 0, time.UTC)}},
