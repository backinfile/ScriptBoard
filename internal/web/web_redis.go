...
func (a *App) saveRedisInstance(response http.ResponseWriter, request *http.Request) {
	...
	instance, err := a.redis.SaveInstance(request.Context(), redismanager.InstanceInput{ID: request.FormValue("id"), Name: request.FormValue("name"), Note: request.FormValue("note"), Environment: redismanager.Environment(request.FormValue("environment")), Host: request.FormValue("host"), Port: port, Username: request.FormValue("username"), Password: request.FormValue("password"), ClearPassword: request.FormValue("clear_password") == "1", TLSMode: redismanager.TLSMode(request.FormValue("tls_mode")), CAPath: request.FormValue("ca_path")})
	if err != nil {
