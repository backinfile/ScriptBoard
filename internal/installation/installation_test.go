...
func TestPrepareCreatesVersionedInstallation(t *testing.T) {
	...
	files := map[string][]byte{
		"scriptboard": []byte("service"), "scriptboard-broker": []byte("broker"), "scriptboard-ai-host": []byte("ai-host"), "scriptboard-updater": []byte("updater"),
		buildinfo.ReleaseInfoFilename: raw,
	}
	if runtime.GOOS == "windows" {
		files = map[string][]byte{
			"scriptboard.exe": []byte("service"), "scriptboard-broker.exe": []byte("broker"), "scriptboard-ai-host.exe": []byte("ai-host"), "scriptboard-tray.exe": []byte("tray"),
