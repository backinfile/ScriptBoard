...
func ServiceAIHostExecutable(metadata Metadata) string {
	name := "scriptboard-ai-host"
	if metadata.OS == "windows" {
		name += ".exe"
	}
	if metadata.OS == "linux" {
...
func ServiceRunnerExecutable(metadata Metadata) string {
	name := "scriptboard-runner"
	if metadata.OS == "windows" {
		name += ".exe"
	}
	if metadata.OS == "linux" {
...
func ValidateVersion(metadata Metadata, version string, want buildinfo.Info) error {
	...
	required := []string{"scriptboard", "scriptboard-broker", "scriptboard-ai-host", "scriptboard-runner", "scriptboard-updater", "install.sh", buildinfo.ReleaseInfoFilename}
	if metadata.OS == "windows" {
		required = []string{"scriptboard.exe", "scriptboard-broker.exe", "scriptboard-ai-host.exe", "scriptboard-runner.exe", "scriptboard-tray.exe", "scriptboard-tray-launcher.exe", "scriptboard-updater.exe", "install.cmd", buildinfo.ReleaseInfoFilename}
	}
