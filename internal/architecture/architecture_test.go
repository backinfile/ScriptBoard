...
func TestLegacyAppPackageCannotReturn(t *testing.T) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, err error) error {
		...
		if entry.IsDir() {
			name := entry.Name()
			// .scratch may contain intentionally old deployment fixtures used by
			// upgrade tests; architecture gates only inspect the live source tree.
			if name == ".git" || name == ".scratch" || name == "node_modules" || name == "dist" {
