...
**固定角色（Fixed Role）**：
用户在整个实例范围内拥有的四种互斥权限之一：系统管理员、维护员、执行员或观察员。角色不是可编辑权限集合；内容分组只组织页面，不参与授权。
...
**任务内存额度（Task Memory Limit）**：
任务及其子进程共享的内存上限。空值继承部署默认值，unlimited 表示不增加任务级限制；仍受 Runner 总额度及 Windows 单进程额度约束。Run 保留启动时解析后的额度快照。
...
**主机条目权限（Host Entry Permissions）**：
某个普通主机文件或目录由操作系统实际执行的访问控制状态。Linux 表示所有者、组和 POSIX 模式；Windows 表示所有者、继承状态和 DACL。ScriptBoard 只经特权 Broker 读取或修改它，不建立独立权限副本。
_Avoid_: ScriptBoard 角色、权限模板、文件共享策略
...
**快捷执行版本（Quick Run Version）**：
快捷执行项可执行配置的单调递增发布编号。外部接口固定绑定快捷执行项、版本和脚本摘要；快捷执行修改后，只有被明确同步的关联接口才改绑到新版本，未同步的旧版本绑定会失效。软锁不属于可执行配置，因此锁定或解锁不改变版本。
_Avoid_: 应用版本、脚本文件版本、锁定次数、外部接口版本

**内容分组（Record Group）**：
维护员在实例范围内定义并排序的一层页面组织容器，由快捷执行、计划、变量、文件快捷访问和网站监控共同使用。每项至多属于一个分组；删除分组前展示五类内容的影响数量，确认后按各集合原有相对顺序追加到派生的“未分组”。分组不参与授权，外部接口分组不属于此目录。
_Avoid_: 快捷执行分组、计划分组、文件夹、标签、执行队列、权限组
...
**计划项（Schedule）**：
由 ScriptBoard 保存和调度的定时脚本执行配置，包含可空内容分组、脚本路径、参数、时间规则和重叠策略。
...
**变量（Variable）**：
由管理员在 ScriptBoard 中预存的具名普通值，具有 `text`、`bool`、`integer`、`float` 或严格 `x.y.z` 的 `version` 类型，可附加一段不参与解析的文字注释，可在参数中通过名称引用，并在启动脚本前解析为字符串。类型只约束写入格式，不提供业务范围。变量可标记为密码类型以便在变量页面默认隐藏，但该标记不改变其明文存储和解析方式。
_Avoid_: 秘密、凭据、环境变量
...
### Inspiration space — 灵感空间

所有登录用户共同查看和编辑的面板集合，保存笔记、链接、待办、计时器与画板。面板是内容模块的有序容器；画板视窗尺寸与笔迹坐标分离，平移只改变观察原点。模块以独立版本号进行比较更新，不同模块的修改可并发保存；面板名称和增删排序单独处理冲突。保存后通过 SSE 通知其他客户端拉取最新状态，正在编辑的本地内容会被保留。

_Avoid_: 自定义面板（该术语专指已有的观测/接口面板）。
