...
**面板操作（Dashboard Action）**：
挂在数据卡片上、由按钮触发的动作定义，随卡片配置保存。快捷执行类操作绑定一个既有快捷执行项并在服务端启动 Run；浏览器直连类操作由访客浏览器直接发起 HTTP 请求，不经过服务端，也不允许保存静态秘密。
_Avoid_: 外部调用路径、Webhook、卡片内嵌脚本

**面板可见性（Dashboard Visibility）**：
自定义面板的四档访问边界：不公开、公开只读、公开可操作、匿名可操作。它取代单一的公开开关；只有公开可操作档持有访问 Key，匿名可操作档只允许浏览器直连操作。
_Avoid_: 角色权限、公开开关、链接分享

**面板访问 Key（Dashboard Access Key）**：
公开可操作面板中访客触发面板操作的凭据。明文仅在生成或轮换时显示一次，库中只保存密封密文与随机提示；连续失败会临时锁定，轮换立即使旧 Key 失效。
_Avoid_: 用户 Session、外部接口 Key、公开地址标识

**访客凭据（Visitor Credential）**：
浏览器直连操作中由访客自己浏览器持有并附加到请求上的请求头值。它只存于访客浏览器的本地存储，不进入面板配置、服务端数据库或公开页 HTML。
_Avoid_: 卡片请求头、面板访问 Key、静态秘密

**流程卡片（Flow Card）**：
以流程配置定义节点 DAG 的自定义面板卡片；节点绑定快捷执行项并按 needs 拓扑串行执行，节点与流程可声明带 when 条件的 post 清理步骤。流程运行状态保存在内存中供实时推送，不持久化历史序列。
_Avoid_: 工作流定义、计划任务、并行编排平台
