...
ScriptBoard 是面向单机、少量可信用户场景的受管目录文件服务器，并为其中的脚本增加执行和追踪能力。它不是通用运维编排平台。
...
**维护员（Maintainer）**：
除用户管理外拥有全部 Web 能力的固定角色，包括文件与执行配置变更、任意 Run 停止、审计、更新和版本保护。
_Avoid_: 第二管理员、自定义权限

**执行员（Operator）**：
可以观察运行状态、读取受管文件、执行受管脚本和全部快捷执行项，并且只能停止自己启动的 Run；不能修改文件、配置、变量、计划或运行一次性源码。
_Avoid_: 受限维护员、分组执行员
