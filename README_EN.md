...
The kubeconfig `server` may use `http://` or `https://`. HTTPS connections validate the kubeconfig CA and client certificate or system trust, and also preserve an explicit `insecure-skip-tls-verify` choice; that option encrypts transport but removes API Server authentication and permits man-in-the-middle attacks. HTTP connections may still use static tokens or basic authentication, but credentials and cluster data travel in plaintext. ScriptBoard does not store kubeconfig tokens, certificates, or private keys, and rejects `exec`/`auth-provider` login plugins. For managed installations, ensure that the Broker identity can read the kubeconfig and any referenced CA, token, or client-certificate files.
...
Back up before upgrading from an older version. The current release uses database schema 51 and can migrate schemas 20–50 automatically; older databases and legacy configuration files are not migrated automatically. Schema 45 adds the durable Registry connection operation log, schema 46 adds its crash-safe completion phase, schema 47 separates External Interface display labels from URL call names, schema 48 converts the single Kubernetes connection and its retained history into independently monitored multiple connections, schema 49 adds Variable value types while migrating existing Variables to `text`, schema 50 adds Variable revisions starting at 1, and schema 51 adds optional Variable notes.
