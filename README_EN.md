...
Back up before upgrading from an older version. The current release uses database schema 50 and can migrate schemas 20–49 automatically; older databases and legacy configuration files are not migrated automatically. Schema 45 adds the durable Registry connection operation log, schema 46 adds its crash-safe completion phase, schema 47 separates External Interface display labels from URL call names, schema 48 converts the single Kubernetes connection and its retained history into independently monitored multiple connections, schema 49 adds Variable value types while migrating existing Variables to `text`, and schema 50 adds Variable revisions starting at 1.
