...
- **Observe the host:** inspect CPU, memory, storage, applications, Docker, Kubernetes, websites, and run history. Kubernetes external access, workloads, and nodes start collapsed, with shared manual and automatic refresh controls above the tabs.
...
Registry connections support HTTP, HTTPS, and explicit certificate verification bypass (a man-in-the-middle risk). Deletion affects every tag sharing a digest; the registry must enable deletion and run garbage collection to reclaim storage. Rules use image creation time. Incomplete or oversized catalogs are rejected before cleanup.
...
Open **Resources → Inspiration space**, select **New space**, and enter a name. Changes save automatically; retry if saving a new space fails. Click a tab once to switch spaces; use **Space actions** on the right to rename or delete. Undo deletion from the bottom toolbar. The floating toolbar switches between View and Edit. Add notes, links, tasks, timers, and canvases, rename blocks, and reorder them. Canvases offer 10 colors, a local eraser, and editable multiline text with font size, color, dragging, deletion, and undo. View mode keeps links, task completion, timers, and canvas panning available.
