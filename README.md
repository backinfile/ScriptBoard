...
管理员和维护员可在“配置 → 外部接口”创建带有效期的 Key。每个 Key 可配置多个以 `name` 区分的功能条目：向指定且受保护路径策略约束的日志文件追加记录、提交单个文件到私有上传收件箱、启动已有快捷执行、按布尔、整数、枚举、短文本约束修改一个非密码变量，或只读开放本实例的网站监控快照。外部文件不会直接进入目标目录；管理员或维护员必须在“资源 → 上传收件箱”核对文件名、目标和 SHA-256 后确认发布。
