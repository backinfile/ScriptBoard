...
“监控 → 概览”默认集中展示本机和已连接 ScriptBoard 机器的在线状态、CPU、内存、存储及网络速率。连接不需要额外 Agent：先在被监控机器的“设置 → 机器连接”创建一次性显示的只读令牌，再在汇总机器的概览中选择“添加机器”，填写名称、ScriptBoard 地址和令牌。保存前会验证连接，之后默认每 15 秒刷新；连接失败时保留最后一次成功快照并标记为未连接。

状态接口只返回经过裁剪的基础资源数据，不包含文件路径、日志、Run 详情、网络接口地址或其他凭据。被监控端只保存令牌的 SHA-256 校验值，汇总端使用 State Root 主密钥密封令牌；令牌可在被监控端随时撤销。地址支持 `https://` 和 `http://`：公网应使用证书可信的 HTTPS，HTTP 仅适合受信任私网，因为令牌和状态会以明文传输。
