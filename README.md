...
- 查看 Docker、Kubernetes（含 NodePort、LoadBalancer 与 Ingress 外部入口）和网站状态；Kubernetes 外部访问、工作负载与节点默认折叠，页签上方统一提供手动刷新和自动刷新开关
...
在“系统设置 → 内存限制”中调整总额度、默认任务额度及平台专属额度；保存后重启服务生效。管理员和维护者可修改；环境变量与命令行参数仍优先。
