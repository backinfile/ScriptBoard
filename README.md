...
v2.14.0 新增面板流程、参数化快捷执行与私有备注，完善镜像仓库管理和移动端体验。详见[发布说明](./docs/RELEASE_NOTES.md)。
...
- 在数据库工作台切换连接，备份和恢复 MySQL/MariaDB，并查看 Redis 数据
- 在“资源 → 镜像仓库”顶部切换 Registry 连接，按 namespace 浏览镜像，查看版本、压缩大小和创建时间；新连接默认只读，可测试连接并切换为允许修改，通过“清理规则”预览批量删除范围
- 管理用户、角色、审计记录和外部调用
