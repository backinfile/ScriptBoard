...
管理员和维护员可在“配置 → 外部接口”创建带有效期的 Key。每个 Key 可配置多个以 `name` 区分的功能条目：记录日志、向固定目录上传单个文件、启动已有快捷执行，或按布尔、整数、枚举、短文本约束修改一个非密码变量。

调用格式为 `POST /trigger?name=ENTRY_NAME`，并通过 `Authorization: Bearer KEY` 传递 Key。完整 Key 只在创建或轮换后显示一次，请保存到密钥管理系统；非本机调用必须使用 HTTPS，不再使用时应停用或轮换。
