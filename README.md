...
提交时按当前用户权限重新校验，外部内容始终按不可信数据处理。当前安全基线以无工具 Pi
运行；只有随受信 Runtime 发布的固定 Tool Broker/Extension 才能增加运维工具，绝不会
回退到 Pi 默认 Shell 或任意文件工具。
...
AI 对话不是安全边界。Provider 能看到发送给它的 Prompt；未来启用的工具仍以当前
