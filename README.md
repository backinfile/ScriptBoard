...
- **保护操作边界**：提供固定角色、审计、主机安全检查、受限外部接口、签名更新和可选 AI 助手。
...
上传多个文件时，ScriptBoard 会先校验整个批次，再统一写入；失败不会留下半个批次。可执行文件和脚本会先进入上传收件箱，确认目标与 SHA-256 后才发布。

## 常用功能

### 快捷执行与计划

快捷执行保存脚本路径、参数、超时和脚本摘要。脚本内容变化后，旧配置会拒绝运行，需由有权限的用户重新发布。计划使用标准五字段 Cron，例如 `0 2 * * *` 表示每天 02:00 运行；服务停止期间错过的计划不会补跑。
...
AI 默认关闭。管理员可安装与当前版本匹配的 Pi Runtime，再配置 OpenAI、Anthropic 或 OpenAI 兼容服务。只有对话内容和用户明确引用的资源会发送给所选模型服务；使用前请确认费用、隐私和数据驻留政策。
