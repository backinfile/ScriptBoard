...
外部接口的每个调用功能都可单独开启“需要审批”（默认关闭）。开启后，变量、日志、文件上传和快捷执行调用先进入“审批”页签；每条审批可在右侧抽屉中查看分类型详情，包括变量修改前后值、待写入日志、快捷执行配置，以及上传缓存内容、目标目录和同名文件处理结果。上传缓存可在审批中直接下载检查；下载不会领取审批或写入目标目录。上传内容只有人工批准后才写入目标文件系统。拒绝、配置变化或恢复失败都不会执行目标动作，批准操作只会领取一次。
