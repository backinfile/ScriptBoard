...
v2.13.1 修复主机安全登录类型筛选，并明确自定义日期范围校验。详见[发布说明](./docs/RELEASE_NOTES.md)。
...
- 浏览、搜索、预览、编辑、上传和下载文件；上传结果逐项显示新创建、已覆盖或已重命名；通过“跳转”输入路径或搜索当前目录、子目录及全部主机位置，自动进入文件所在分页并高亮定位
...
- Kubernetes 负载表可独立显示完整镜像（含标签或摘要）和关联容器上次启动时间；通过“显示列”选择列，设置仅保存在当前浏览器，宽表可横向滚动。
- 数据库与镜像仓库使用一致的连接侧栏和内容布局；在数据库工作台中备份和恢复 MySQL/MariaDB，并查看 Redis 数据
- 在“资源 → 镜像仓库”切换 Registry 连接，在镜像内容区折叠浏览多级 namespace，首次点击节点筛选此前缀的镜像，再次点击当前节点展开或收起子目录，查看版本标签、压缩大小和版本时间；连接可切换只读/允许修改，从“清理规则”批量删除镜像，新连接默认只读，左侧纵向切换连接，内容区右上角可编辑当前连接
- 管理用户、角色、审计记录和外部调用
- 主机安全登录记录支持按 SSH 密码、公钥或 Windows RDP、网络登录类型筛选

登录记录的自定义日期需同时填写起止日期，跨度最多 31 天；清空两个日期可使用快捷时间范围。
...
MySQL 连接默认打开本地备份；实例离线时仍可查看、下载备份，以及查看计划和执行记录。备份文件由浏览器原生下载。点击数据库连接可切换实例，悬停显示完整名称和连接地址。点击备份计划名称可查看 Cron、未来五次执行时间和分页执行记录。

Redis 键空间支持选择冒号（`:`）或双冒号（`::`）分段，默认使用双冒号；分页和键值预览保留所选模式。
