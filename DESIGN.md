...
Status, language, and settings controls live at the bottom. The wordmark is plain text and may use the instance display name configured by an administrator or maintainer; long names truncate within the fixed sidebar. Settings has five intent-based categories: My account, Users, Instance, Connections and notifications, and Maintenance. Each category and entry retains its own permission check. Category links wrap on narrow screens. Editors open in the shared task drawer on desktop and mobile, with complete GET pages for direct access and without JavaScript. Browser display preferences are available to every signed-in user and never change monitoring truth.
...
The Files workspace begins at “This host”: Windows shows available local, removable, and network volumes, while Linux enters `/`. Breadcrumbs, search, forms, and browser-local Quick access all use canonical absolute paths. Quick access is versioned, renders one folder per row, and starts collapsed; invalid or protected pins can only be removed. Hidden entries are off by default (including Windows Hidden/System attributes), and the toolbar switch preserves search and sort state. The desktop list keeps a real header for Name, Size, Created, and Modified; each header toggles server-backed ascending and descending order while Linux shows an unavailable Created value when the filesystem exposes no birth time. An explicit multi-select mode reveals row checkboxes, Shift-range selection, select-all, a live count, copy paths, ZIP download, move, and recoverable Trash actions without changing ordinary row navigation. Batch move uses the directory tree and stops the whole request before mutation if any destination name conflicts; batch Trash confirms impact before disabling related schedules. Protected paths never appear, while links, reparse points, special files, and kernel virtual filesystems remain visible only as restricted entries. New directory and Upload files form one header action group, with Upload files as the single primary action. The complete browser surface accepts dragged files for immediate upload into the current path and reveals its boundary and status only while a drag, upload, or error is active. Upload, move, rename, and Trash restore never overwrite a same-name item by default; a focused conflict dialog offers skip, recoverable overwrite, or rename. Cross-filesystem moves expose scan, byte progress, commit, and failure states without hiding the current workspace. Destructive actions remain secondary and explicit, with Trash at the far edge of the file toolbar.
