...
Ubuntu 26.04 的真实 systemd 安装已经验证完整 stop/start、四个主进程逐一 `SIGKILL` 后自动恢复，以及 AI/Runner 不能读取应用数据库和实例主密钥。发布平台仍须实际验证所支持 Windows 版本上的服务生命周期，并完成两平台的主动网络阻断与资源耗尽矩阵；不能仅以 unit 文本或 mock 测试代替这些门禁。
