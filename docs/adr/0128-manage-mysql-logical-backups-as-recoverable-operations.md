...
在受管部署中，备份产物的目录准备、生成、上传落盘、摘要与 gzip 校验、读取、删除、轮换和残留清理均由特权 Broker 完成；低权限 Web 进程只保存路径与 Broker 返回的产物元数据，不直接打开备份文件。产物继续保持私有权限，不通过放宽 POSIX mode 或 Windows ACL 解决跨身份访问。
