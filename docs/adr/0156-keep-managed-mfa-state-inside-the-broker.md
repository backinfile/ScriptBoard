...
该决定只迁移 MFA 领域，不代表 P0-02 已完成。Passkey、Assistant Provider、MySQL、External Interface/远程连接凭据以及 Host Files 特权仍需分别改造成验证、代理或执行型接口；不得以返回任意明文的通用 Broker RPC 代替这些迁移。
