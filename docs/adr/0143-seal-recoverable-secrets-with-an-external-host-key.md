...
MySQL 密码和远程 ScriptBoard 网站状态 Key 必须能够在服务运行时恢复，
...
启动迁移覆盖 State Root 内的 MySQL AES key，以及 External Interface/远程网站秘密的旧 AES
key。新密文完整提交后先删除旧原始 key，再删除旧密文。启动不能完成迁移时 fail
...
后续 ADR-0156、ADR-0157 与 ADR-0158 已分别把受管部署的 MFA、Passkey 和远程网站连接状态迁入 Broker-owned 领域存储；本决策中的通用外部主密钥在受管 Web 内只继续服务 MySQL，并保留非受管部署的兼容路径。
