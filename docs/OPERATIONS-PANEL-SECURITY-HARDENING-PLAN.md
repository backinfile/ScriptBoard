...
| P0-02 | 八个迁移切片完成 | 正式包新增独立 Broker、AI Host 与 Runner 受管服务。固定宿主写动作只经 Broker；受管 Linux Web 使用无登录 `scriptboard-web`，Windows Web 使用独立服务 SID 的 `LocalService`。AI 与 Run 分别使用独立 OS 身份和受保护 IPC：Web 不再持有其子进程，Runner 会在启动点复核脚本摘要、工作目录和自己配置的 executor。受管 Web 不再解封或持有审计 checkpoint 私钥，也不再打开或解封 TOTP/恢复码、Passkey 公钥凭据及远程网站连接 Key；这些领域只经固定 Broker 操作，连接 Endpoint 与 Key 原子绑定且由 Broker 注入 Authorization 和执行受控 HTTPS 获取，Web 只能收到经过 schema 与大小验证的响应。凭据变更重新验证近期会话并独立审计，远程连接读取也重新验证有效观察会话；TOTP 验证在 Broker 内有界限流，Passkey 计数器更新禁止替换注册身份字段。Ubuntu 26.04 的真实 systemd 安装已验证 AI/Runner 无法读取应用数据库和主密钥；Web 仍因 Provider 与 MySQL 兼容消费者而可读取通用主密钥，Host Files 特权也仍在 Web 进程/身份内，因此 P0-02 验收仍未完成。 |
...
| P0-12 | 部分完成 | 增加 vet、race、govulncheck、CodeQL、secret scan、SBOM 与 release provenance 门禁；race 覆盖 App、审计链、外部能力、Run、受控进程、上传/导入、出站策略及 Assistant Runtime/Provider 代理等并发安全边界。已加入出站地址、Host 和命令参数 fuzz target；故障测试覆盖审计事务回滚、远端事件 503 重试、检测基数、更新数据库快照截断、恢复 staging 校验、Linux 当前版本指针/metadata 之间的中断修复，以及 schema 41→42 State Root 迁移中途失败时的事务回滚。Ubuntu 26.04 已验证四服务逐一 `SIGKILL` 和完整 stop/start 恢复；真实主机断电、Windows SCM 与跨文件系统平台故障矩阵继续补充。 |
...
- OS 权限探针确认 `scriptboard-ai` 与 `scriptboard-runner` 无法读取应用数据库或实例凭据主密钥；`scriptboard-web` 仍能读取主密钥，这是 P0-02 的已知未完成项。
...
| 密钥保存 | Provider 与 MySQL 使用用途绑定 AES-GCM，主密钥在 State Root 外；TOTP、Passkey 与 Endpoint 绑定的远程网站 Key 已迁入 Broker-owned 领域存储；Windows 主密钥再由机器级 DPAPI 保护 | 单独窃取 State Root 不能离线解密；受管 Web 不再读取已迁移领域状态；Unix 外部 key 仍须作为 root-only 秘密独立备份 |
