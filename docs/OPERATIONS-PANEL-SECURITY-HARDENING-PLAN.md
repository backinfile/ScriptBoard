...
- [ ] P1 OS 安全更新只读视图、服务日志、State Root 备份、安全基线和通知。State Root 备份的带外 CLI 纵向切片已完成：一致性 SQLite snapshot、固定私有状态白名单、Argon2id + XChaCha20-Poly1305 分块认证加密、逐文件清单、签名审计 checkpoint、恢复前 staging/完整性/schema 校验、Session 撤销、失败回滚、恢复前状态保留与受控审计重锚均有自动化测试；Web step-up/Broker staging、整机丢失外部密钥恢复演练及其余四项 P1 能力仍待完成。
