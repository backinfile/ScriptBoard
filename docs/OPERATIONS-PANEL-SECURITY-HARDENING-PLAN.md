...
创建日期：2026-08-11  
适用范围：ScriptBoard Web 服务、脚本执行、文件管理、网站监控、外部接口、MySQL、更新与服务安装
...
ScriptBoard 已有一批值得保留的安全基础：Argon2id 密码哈希、服务端会话、CSRF、CSP/HSTS、请求体和超时限制、非回环监听强制 TLS、受保护路径、文件原子替换与回收站、参数化 SQL、更新包 Ed25519 签名与摘要校验、归档路径穿越防护，以及外部 Key 哈希与限流。

后续产品决策由 ADR-0175 调整了 P0-07 的一项策略：MFA 注册不再设置截止时间，schema 53 删除旧字段；高风险操作的 step-up 保持不变，已配置 MFA 的账户仍必须使用第二因素。

原始审阅确认最重要的问题不是缺少更多运维按钮，而是 Web 与脚本执行共享高权限身份。当前实现已经将其拆为低权限 Web、特权 Broker 与独立 Runner，并把三个组件绑定成同一版本化产品；剩余工作是由正式发布平台执行真实服务身份、ACL、崩溃/断电、签名轮换和外部集成矩阵，不能再通过回退到高权限单体来简化部署。
...
本计划方向正确，但不能作为单个提交或单个发布周期整体执行。P0-02 会直接取代 ADR-0023 的最高宿主权限模型，并影响主机文件管理、安装器、更新器和受信脚本语义；P0-07、P0-10 与 P0-11 也分别需要数据迁移、发布基础设施和运维演练。它们必须保留为独立批次，不能用未验证的“加固配置”假装完成。
...
| P0-02 | 十一个迁移切片完成 | 正式包新增独立 Broker 与 Runner 受管服务。固定宿主写动作只经 Broker；受管 Linux Web 使用无登录 `scriptboard-web`，Windows Web 使用独立服务 SID 的 `LocalService`。Run 使用独立 OS 身份和受保护 IPC，Runner 会在启动点复核脚本摘要、工作目录和 executor。受管 Web 不再持有审计 checkpoint 私钥、MFA/Passkey 状态、远程网站连接 Key、MySQL 密码或宿主文件；这些领域只经固定 Broker 操作。Host Files、MySQL 与远程连接均由 Broker 在执行点复核，凭据变更重新验证近期会话并独立审计。Ubuntu 26.04 的真实 systemd 安装已验证 Runner 无法读取应用数据库和主密钥；Windows SCM 身份、精确 demand-start ACL、启动凭据 ACL、崩溃恢复和卸载矩阵已有提升权限 CI 门禁，仍须在正式发布运行中取得通过证据。 |
...
| P0-04 | 五个切片完成 | 建立共享出站策略并覆盖网站 HTTP/WebSocket 探测、远程监控聚合、GitHub 更新检查/下载及 Custom Dashboard 数据源；这些默认客户端不使用环境代理，DNS 解析后的实际 IP 由受控 Dialer 固定并拒绝私网、元数据和非常规端口。Dashboard 数据源禁止重定向、URL 内嵌凭据及保留请求头；远程 ScriptBoard 聚合只接受 HTTPS，网站探测跳过 TLS 验证的例外最长一小时并记录到期时间。 |
...
| P0-12 | 部分完成 | 增加 vet、race、govulncheck、CodeQL、secret scan、SBOM 与 release provenance 门禁；正式 Tag 的归档构建现直接依赖同一 Tag commit 上的全量测试/vet/build、关键安全包 race、安全边界 fuzz、Chromium、govulncheck、gitleaks、CodeQL 与提升权限 Windows SCM 三服务矩阵，普通分支 CI 的历史结果不能替代发布门禁。race 清单由 CI 与 Release 共用，并覆盖 App、审计链/通知、外部能力、Run/Runner、Broker、Host Files、State Backup、更新、受控进程、上传/导入及出站策略等并发安全边界。出站地址、Host 和命令参数 fuzz target 现在由安全工作流及每个正式 Tag 各执行 30 秒，而不再只运行 seed corpus；故障测试覆盖审计事务回滚、远端事件 503 重试、检测基数、更新数据库快照截断、恢复 staging 校验、Linux 当前版本指针/metadata 之间的中断修复，以及 schema 41→42 State Root 迁移中途失败时的事务回滚。Ubuntu 26.04 已验证三服务逐一 `SIGKILL` 和完整 stop/start 恢复；真实主机断电、Windows SCM 与跨文件系统平台故障矩阵继续补充。 |
...
P0-02 与 P0-11 的代码切片已经完成；P0-10/P0-12 的正式签名轮换演练、真实断电、跨文件系统故障和 SIEM 产品矩阵，以及 P2 未在本分支宣称完成。Windows SCM 提升权限门禁已经进入 CI，但本机当前令牌不具备管理员权限，因此本地 `--development-current-user` 证据仍不能替代正式门禁结果。

### 1.1 本地平台门禁记录（2026-08-12）

- 在 Ubuntu 26.04 LTS（WSL2、systemd）从正式发布归档布局安装 Broker、Runner 与 Web，三个 unit 均以声明的独立身份启动，`systemd-analyze verify` 通过，Web readiness 返回 HTTP 200。
- 对三个主进程逐一发送 `SIGKILL`，systemd 均按策略自动恢复；随后执行完整 stop/start，三个 unit 再次 active。测试暴露并修复了服务入口未处理 `SIGTERM`、Broker 无法安全接管授权 Web 身份遗留 socket 的真实生命周期缺陷。
- Web unit 的 `MemoryDenyWriteExecute=yes` 已由实际 unit 属性确认。主动网络探针确认 Runner 不能创建 TCP socket；主动资源探针确认 `TasksMax` 拒绝超额进程。内存探针发现单独使用 `MemoryMax` 时工作集可转入默认无限 swap，因而为 Runner 补充 `MemorySwapMax=0`；修复后同一超限探针被 cgroup 终止，实际 unit 属性也确认 swap 上限为零。
- OS 权限探针确认 `scriptboard-runner` 无法读取应用数据库或实例凭据主密钥；MySQL 兼容路径现已迁入 Broker，下一轮真实安装需复验 `scriptboard-web` 也无法读取主密钥。
...
- Windows 本地三进程部署通过登录、远端 Runner 一次性 Run、Quick Run 资源 revision/digest 审计和 Chromium 桌面门禁；随后在 `127.0.0.1:11149` 的外部 Chrome 复验手动 Host Files Run，执行身份为 `scriptboard-runner`，输出成功且审计 SHA-256 与磁盘脚本摘要一致。Windows SCM 安装需要提升权限，本轮未执行。
- 同一 Windows 三进程部署升级到 Broker v7 / Web fixture v10 后，在外部 Chrome 创建了仅指向不可达环回端口的 `Broker MySQL 探针`：页面只显示非秘密连接元数据和受控失败，不回显密码。精确停止 Broker 后，Web 保留元数据但特权调用 fail closed；重启同一 Broker 构建后可重新读取实例。Broker 生成 `mysql-credentials.v2.enc` 与端点绑定密文，探针明文不在两份密文中，旧 `mysql-credentials.key` / `mysql-credentials.enc` 均不存在，Web 与 Broker stderr 均为空。自动化集成测试另以分离的 Web/Broker 凭据根验证只有 Broker 产生 MySQL 密文，并覆盖端点替换、路径逃逸、固定协议、客户端 allowlist、跨进程单活动操作和取消传播。
- 同一三进程部署升级到 Broker-owned MFA 后，外部 Chrome 可正常读取 MFA 设置页；精确停止 Broker 时该页面返回受控 HTTP 500，Broker 恢复后立即可用，未回退到 Web 本地 MFA store。隔离集成测试还使用不同的 Web/Broker 凭据根确认 MFA 密文只在 Broker 侧产生。
- Broker-owned Passkey 隔离测试以不同凭据根确认 Web 可列出 Broker 侧公钥凭据且本地不产生 Passkey 密文；协议测试覆盖无会话、过期 step-up、跨用户调用、通用 payload、公钥替换与 Broker 内 TOTP 限流。本切片重新部署 Windows 三进程后，外部 Chrome 完成登录、MFA/Passkey 页面读取与受会话保护的 MFA 注册初始化；精确停止 Broker 时页面受控返回 HTTP 500，重启后同一会话恢复。随后手动 Host Files Run 输出 `RESOURCE_BINDING_V2_OK`，执行身份仍为 `scriptboard-runner`。
- Broker-owned 远程网站连接隔离测试以不同凭据根确认 Web 页面可保存并使用 Broker 侧 Endpoint/Key 绑定，而 Web 根不产生该密文；协议测试覆盖无会话、过期会话、通用 payload、字段混用、HTTPS/Key 形状、响应上限和重定向不转发凭据。Windows 三进程升级后，外部 Chrome 创建只显示 Endpoint 的测试连接；Broker 下线时删除受控返回 HTTP 500 且元数据保留，重启后同一会话成功清理。随后手动 Host Files Run 再次输出 `RESOURCE_BINDING_V2_OK`，执行身份为 `scriptboard-runner`，浏览器控制台无错误。
- 收口切片新增 `service verify` 与提升权限 `windows-scm-security-gate.ps1`：复核一套 Installed Release 的三个服务路径、同一 State Root、账户、SID 类型、启动模式、依赖、有界恢复动作及 Web 对 Runner 的精确 `START + QUERY_STATUS` ACE；实际启动后还验证 Web/Broker 常驻、Runner demand-start、Named Pipe 拒绝管理员旁路、Broker-only 目录和令牌 ACL、三服务强杀恢复、整组停启和卸载无残留。安装器显式向 Web 自有的管理员 password file 与 TLS 材料授予最小只读权限，不向 Broker relay token 扩权。当前非提升本机执行在任何变更前按预期拒绝；门禁已接入 Windows CI，正式结果以提升权限 runner 为准。
...
- 部署入口先收敛为 `install.cmd`/`install.sh`，随后进一步替换为每个平台一个原生自解包文件：Windows `*-setup.exe`、Linux `.run`。启动器附加完整 ZIP 载荷，首次安装与自动更新复用同一套摘要、路径、类型、数量、解压大小和 `RELEASE.json` 验证，再统一调用 `service install --start`；`--extract-to` 保留便携目录能力。发布清单直接绑定四个平台安装器，updater protocol 提升为 2 并拒绝把 protocol 1 伪装为兼容。三个组件的内部边界、整体版本、事务切换与回滚语义不变。
...
目标架构拆为三个进程信任边界，并作为一套发布包、一个版本号整体管理：
...
部署边界按 ADR-0163 收敛：一套发布包只对应一个 ScriptBoard 版本，manifest 同时绑定 Web、Broker 与 Runner 的二进制摘要和 IPC 协议；安装、升级、回滚和卸载整体执行，混合版本 fail closed。Web 与 Broker 常驻；Linux Runner 使用 systemd socket activation，Windows 使用 SCM demand-start 并在首次 IPC 前启动服务。按需启动不能成为把执行能力合并回 Web 的理由。Windows `--development-current-user` 不属于正式部署；真实 SCM 服务身份、Named Pipe DACL、生命周期和整体版本门禁已自动化，仍须由提升权限发布 runner 执行。
...
| 软件更新 | PackageKit 或 OSTree 管理系统包 | 只管理 ScriptBoard 的签名更新/回滚 | P1 增加 OS 安全更新只读状态；系统包安装必须使用发行版 API 和独立 Broker |
...
| CyberPanel 2.3.5 安全公告 | WebTerminal、CloudAPI、上传、命令过滤和 API Token 多处鉴权旁路；有接口在鉴权完成前先执行命令 | 文件上传、外部接口、系统操作 | 移除通用终端；不依赖黑名单式 `commandInjectionCheck`；公开入口只映射固定动作；Token 最小权限、过期和轮换；所有副作用位于鉴权之后 |
...
| aaPanel CVE-2026-29859 | 任意文件上传最终造成代码执行 | 文件管理、外部上传、数据库导入 | 上传写入非执行目录；文件名与目标路径由服务端决定；内容/大小/格式校验；隔离、原子发布、禁止上传后自动执行；执行文件另走签名发布流程 |
...
| Portainer CVE-2026-72533 | 授权层规范化后的路径与实际转发路径不一致；三段版本号和编码分隔符跳过全部 Docker API 授权，并使既有修复重新可绕过 | `ServeMux`、RouteSpec、External Interface 签名 | 请求目标先得到唯一规范表示；授权、审计、签名与执行只使用同一表示；拒绝编码分隔符、反斜杠、点段和歧义转义并建立 fuzz/黑盒回归 |
| Portainer / Coolify / Pterodactyl 2026 公告 | 鉴权失败后继续、空 HMAC 允许部署、跨资源 IDOR、权限变化后旧会话继续、归档穿越和符号链接读取 | 外部入口、Fleet capability、长连接、更新/恢复、Host Files | 认证失败副作用哨兵；空/重复/过期/撤销凭据 fail closed；owner/connection/revision 原子绑定；撤销传播；共享恶意归档和链接语料 |

本轮增量已落地以下回归边界：

- 请求进入路由前拒绝编码分隔符、反斜杠、点段、重复斜杠和 absolute-form 等歧义目标；已退役的 GET Trigger 不再登记。
- External Interface 对重复或并存的认证/签名 Header fail closed，拒绝发生在读取业务载荷或触发上游副作用之前。
...
| 密钥保存 | MySQL 使用用途绑定 AES-GCM，主密钥在 State Root 外；TOTP、Passkey 和 Endpoint 绑定的远程网站 Key 已迁入 Broker-owned 领域存储；Windows 主密钥再由机器级 DPAPI 保护 | 单独窃取 State Root 不能离线解密；受管 Web 不再读取已迁移领域状态；Unix 外部 key 仍须作为 root-only 秘密独立备份 |
...
- 更新使用签名 Manifest、摘要、文件数/展开大小限制、重复目标和路径穿越检查，且拒绝符号链接等危险归档项。
...
### P0-02 拆分 Web、特权 Broker 和 Run Worker 身份
...
- Run Worker 使用受限 Token、Job Object 资源限制和独立 ACL；禁止访问服务密钥目录。
...
- 获取 Web 进程代码执行后，仍不能调用未授权 Broker 动作或读取 Run 的私有状态。
...
- 建立共享 `OutboundPolicy` 和受控 Dialer，供网站监控、远程监控、更新、Webhook/未来集成使用。
...
- 普通文件管理上传、外部上传、数据库导入、监控配置导入和 Dashboard 导入分别建立独立策略，不能共享“任意文件”默认值。
...
- MySQL 和其他可恢复秘密使用 Windows DPAPI/Credential Manager、Linux systemd credentials/kernel keyring 或独立于 State Root 的 root-only master key；文档说明 State Root 备份是否包含可恢复密钥。
...
- 高风险事件至少包括登录/MFA、角色与用户、外部 Key、脚本发布与执行、文件上传/下载/发布、数据库恢复、系统安全设置、更新和秘密读取。
...
- 建立告警：大量认证失败、未知方法、权限拒绝激增、SSRF 阻断、签名失败、审计链断裂、异常高频 Trigger 和 Runner 越权。
...
| 资源与执行配额 | CPU、内存、进程数、日志字节、并发、磁盘写入和最长运行时间 | 对 Run、上传、导入、监控分别限额，不能只使用全局并发 |
