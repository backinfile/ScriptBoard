...
| P0-02 | 七个迁移切片完成 | 正式包新增独立 Broker、AI Host 与 Runner 受管服务。固定宿主写动作只经 Broker；受管 Linux Web 使用无登录 `scriptboard-web`，Windows Web 使用独立服务 SID 的 `LocalService`。AI 与 Run 分别使用独立 OS 身份和受保护 IPC：Web 不再持有其子进程，Runner 会在启动点复核脚本摘要、工作目录和自己配置的 executor。受管 Web 不再解封或持有审计 checkpoint 私钥，也不再打开或解封 TOTP/恢复码及 Passkey 公钥凭据状态；MFA 与 Passkey 只经固定 Broker 领域操作，凭据变更重新验证近期会话、绑定同一用户并独立审计，TOTP 验证在 Broker 内有界限流，Passkey 计数器更新禁止替换注册身份字段。Ubuntu 26.04 的真实 systemd 安装已验证 AI/Runner 无法读取应用数据库和主密钥；Web 仍因 Provider、MySQL、External Interface/远程连接等兼容消费者而可读取通用主密钥，Host Files 特权也仍在 Web 进程/身份内，因此 P0-02 验收仍未完成。 |
