...
- 部署入口进一步收敛为“一个产品、一次安装”：Windows/Linux 正式归档分别新增 `install.cmd` 与 `install.sh`，统一调用 `service install --start`；主 CLI 在服务注册后自动复用完整 `service verify` 合同，确认同一 Installed Release、配置和四组件服务定义后才启动并输出产品版本与整体状态。安装入口被纳入正式发布包必需文件校验，用户正常路径不再需要手工串联 install、start、status、verify；显式命令继续保留用于高级诊断。四平台 development 归档已实际构建并核对入口和四组件内容，全量 Go 测试、vet 与安装脚本语法检查通过。
