...
| P0-02 | 九个迁移切片完成 | 正式包新增独立 Broker、AI Host 与 Runner 受管服务。固定宿主写动作只经 Broker；受管 Linux Web 使用无登录 `scriptboard-web`，Windows Web 使用独立服务 SID 的 `LocalService`。AI 与 Run 分别使用独立 OS 身份和受保护 IPC：Web 不再持有其子进程，Runner 会在启动点复核脚本摘要、工作目录和自己配置的 executor。受管 Web 不再解封或持有审计 checkpoint 私钥，也不再打开或解封 TOTP/恢复码、Passkey 公钥凭据、远程网站连接 Key 及 Assistant Provider 凭据；这些领域只经固定 Broker 操作。远程连接 Endpoint 与 Key 原子绑定并由 Broker 执行受控 HTTPS 获取；Provider 的 Owner/共享策略、Endpoint、模型与 Key 同样原子绑定，Broker 只返回短期环回代理地址、模型能力和随机撤销句柄。凭据变更重新验证近期会话并独立审计，观察与 Provider 代理启动重新验证有效会话；TOTP 验证在 Broker 内有界限流，Passkey 计数器更新禁止替换注册身份字段。Ubuntu 26.04 的真实 systemd 安装已验证 AI/Runner 无法读取应用数据库和主密钥；Web 仍因 MySQL 兼容消费者而可读取通用主密钥，Host Files 特权也仍在 Web 进程/身份内，因此 P0-02 验收仍未完成。 |
...
- OS 权限探针确认 `scriptboard-ai` 与 `scriptboard-runner` 无法读取应用数据库或实例凭据主密钥；`scriptboard-web` 仍因 MySQL 兼容路径能读取主密钥，这是 P0-02 的已知未完成项。
