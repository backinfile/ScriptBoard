...
| P0-02 | 四个迁移切片完成 | 正式包新增独立 Broker、AI Host 与 Runner 受管服务。固定宿主写动作只经 Broker；受管 Linux Web 使用无登录 `scriptboard-web`，Windows Web 使用独立服务 SID 的 `LocalService`。AI 与 Run 分别使用独立 OS 身份和受保护 IPC：Web 不再持有其子进程，Runner 会在启动点复核脚本摘要、工作目录和自己配置的 executor。Ubuntu 26.04 的真实 systemd 安装已验证 AI/Runner 无法读取应用数据库和主密钥；Web 仍可读取主密钥，Host Files 与凭据解封也仍在 Web 进程/身份内，因此 P0-02 验收仍未完成。 |
...
- Web unit 的 `MemoryDenyWriteExecute=yes` 已由实际 unit 属性确认；AI/Runner 的地址、内存和 task 限额由实际 unit 属性确认。网络阻断和资源耗尽尚未做主动突破测试，因此不据此关闭 P0-08。
...
- 同一提交在 Windows 本地四进程部署中通过登录、远端 Runner 一次性 Run、Quick Run 资源 revision/digest 审计和 Chromium 桌面门禁；Windows SCM 安装需要提升权限，本轮未执行。
