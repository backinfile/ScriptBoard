...
| P0-02 | 十个迁移切片完成 | 正式包新增独立 Broker、AI Host 与 Runner 受管服务。固定宿主写动作只经 Broker；受管 Linux Web 使用无登录 `scriptboard-web`，Windows Web 使用独立服务 SID 的 `LocalService`。AI 与 Run 分别使用独立 OS 身份和受保护 IPC：Web 不再持有其子进程，Runner 会在启动点复核脚本摘要、工作目录和自己配置的 executor。受管 Web 不再解封或持有审计 checkpoint 私钥，也不再打开或解封 TOTP/恢复码、Passkey 公钥凭据、远程网站连接 Key、Assistant Provider 凭据及 MySQL 密码；这些领域只经固定 Broker 操作。MySQL 的直接连接、client option file、`mysql`/`mysqldump` 启动、定时备份与中断恢复也迁入 Broker，实例字段与密码原子绑定，路径、客户端可执行文件和跨进程单活动操作均在执行点复核。远程连接 Endpoint 与 Key 原子绑定并由 Broker 执行受控 HTTPS 获取；Provider 的 Owner/共享策略、Endpoint、模型与 Key 同样原子绑定，Broker 只返回短期环回代理地址、模型能力和随机撤销句柄。凭据变更重新验证近期会话并独立审计，观察与 Provider 代理启动重新验证有效会话；TOTP 验证在 Broker 内有界限流，Passkey 计数器更新禁止替换注册身份字段。Ubuntu 26.04 的真实 systemd 安装已验证 AI/Runner 无法读取应用数据库和主密钥；当前剩余 P0-02 结构性缺口是 Host Files 特权仍在 Web 进程/身份内。 |
...
P0-02 的 Host Files 边界、P0-08 至 P0-11 的其余平台/发布矩阵以及 P1/P2 未在本分支宣称完成。
