...
schema 38 增加以下实例级表；schema 47 将连接改为多条记录，并为历史表增加 `connection_id`。每个连接拥有稳定 ID；工作负载在连接范围内以 `namespace/kind/name` 为稳定键，不使用短生命周期 Pod 或容器 ID。
...
保存连接时若 API Server/CA 指纹改变，只清空该连接在 `kubernetes_versions` 与 `kubernetes_metric_minutes` 中的记录，避免不同集群共享身份和时间线，同时保留其他连接的历史。schema 38–46 的单例连接升级时使用固定旧连接 ID，并把已有历史归入该连接。
